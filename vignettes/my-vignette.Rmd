...
```{r}
# (1) PHS
log_rank <- survdiff(Surv(PFSDYCR) ~ ATRT, rho = 0, data = dt) # fit survival model
...
```{r}
# (2) survival rate
log_rank <- survdiff(Surv(DTHDY, DTH) ~ ATRT, rho = 0, data = dt) # fit survival model
log_rank$chisq # test statistic
1 - pchisq(q = log_rank$chisq, df = 1) # p-value
```
...
ggsurvplot(f2, dt, title = "survival curves for the four treatment*type groups") +
...
```{r}
# Mutant patients
log_rank <- survdiff(Surv(DTHDY, DTH) ~ ATRT, rho = 0, data = dt[which(dt$marker == "Mutant"), ])
...
```{r}
# Wild-type patients
log_rank <- survdiff(Surv(DTHDY, DTH) ~ ATRT, rho = 0, data = dt[which(dt$marker == "Wild-type"), ])
log_rank$chisq # log-rank test statistic
1 - pchisq(q = log_rank$chisq, df = 1) # p-value
```
